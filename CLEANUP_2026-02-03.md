...
- ⏳ DBC service integration needed (TODO in code)

---

### 2. Removed Unused/Duplicate Endpoints ✅
...
### Before Cleanup:
```
/v1/tokens          (public-api)  ←  Frontend
/api/v1/tokens      (meteora-api) ←  Duplicate!

/v1/trade           (public-api)  ←  Frontend
/api/v1/trade       (meteora-api) ←  Duplicate!

/api/v1/transaction (meteora-api) ←  Unused
/api/v1/lp          (meteora-api) ←  Automated
/rewards            (meteora-api) ←  Unused
```
...
### High Priority 🔴
1. **Wire up DBC Service** - Token creation currently throws "not implemented"
   - Inject `DbcService` into `TokenService`
   - Call `dbcService.buildCreateTokenTransaction()`
   - Test end-to-end token creation flow
...
**Commit:** `8dc0464` - "Token creation returns unsigned transaction + remove unused endpoints"
...
**Date:** 2026-02-03  
**Time:** 20:15 UTC  
**Status:** ✅ Cleanup complete, DBC wiring needed
